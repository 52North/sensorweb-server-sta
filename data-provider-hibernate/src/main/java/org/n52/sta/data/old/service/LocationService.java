...
import org.n52.sta.data.old.query.LocationQuerySpecifications;
import org.n52.sta.data.old.repositories.EntityGraphRepository;
import org.n52.sta.data.old.repositories.LocationEncodingRepository;
import org.n52.sta.data.old.repositories.LocationParameterRepository;
...
public class LocationService
        extends CommonSTAServiceImpl<LocationRepository, LocationDTO, LocationEntity> {
    ...
    private void processThings(LocationEntity location) throws STACRUDException {
        if (location.hasThings()) {
            ...
            for (PlatformEntity newThing : location.getThings()) {
                // The only way for a Thing to be processed is if we are currently persisting
                // said Thing
                // IF this is the case the Thing takes care of Locations itself and we must not
                // mess with it here
