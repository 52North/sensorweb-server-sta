...
public class ThingQuerySpecifications extends EntityQuerySpecifications<PlatformEntity> {
    ...
    public Specification<PlatformEntity> withLocationStaIdentifier(String locationIdentifier) {
        return (root, query, builder) -> {
            final Join<PlatformEntity, LocationEntity> join = root.join(PlatformEntity.PROPERTY_LOCATIONS,
                    JoinType.INNER);
            return builder.equal(join.get(DescribableEntity.PROPERTY_STA_IDENTIFIER), locationIdentifier);
        };
    }

    public Specification<PlatformEntity> withHistoricalLocationStaIdentifier(String historicalIdentifier) {
        return (root, query, builder) -> {
            final Join<PlatformEntity, HistoricalLocationEntity> join = root
                    .join(PlatformEntity.PROPERTY_HISTORICAL_LOCATIONS, JoinType.INNER);
            return builder.equal(join.get(DescribableEntity.PROPERTY_STA_IDENTIFIER), historicalIdentifier);
        };
    }

    public Specification<PlatformEntity> withDatastreamStaIdentifier(String datastreamIdentifier) {
        return (root, query, builder) -> {
            final Join<PlatformEntity, AbstractDatasetEntity> join = root.join(PlatformEntity.PROPERTY_DATASETS,
                    JoinType.INNER);
    ...
    @Override
    protected Specification<PlatformEntity> handleRelatedPropertyFilter(String propertyName,
            ...
            try {
                switch (propertyName) {
                    ...
                    case StaConstants.HISTORICAL_LOCATIONS:
                        ...
                        Root<HistoricalLocationEntity> historicalLocation = subquery
                                .from(HistoricalLocationEntity.class);
                        subquery.select(historicalLocation.get(HistoricalLocationEntity.PROPERTY_ID))
                                .where(((Specification<HistoricalLocationEntity>) propertyValue).toPredicate(
                                        ...
                                        builder));
                        final Join<PlatformEntity, HistoricalLocationEntity> join = root
                                .join(PlatformEntity.PROPERTY_HISTORICAL_LOCATIONS);
                        return builder.in(join.get(DescribableEntity.PROPERTY_ID)).value(subquery);
                    default:
    ...
    @Override
    protected Specification<PlatformEntity> handleDirectPropertyFilter(
            String propertyName,
