...
import org.n52.sta.data.old.query.HistoricalLocationQuerySpecifications;
import org.n52.sta.data.old.repositories.EntityGraphRepository;
import org.n52.sta.data.old.repositories.HistoricalLocationRepository;
...
public class HistoricalLocationService
        extends
        CommonSTAServiceImpl<HistoricalLocationRepository, HistoricalLocationDTO, HistoricalLocationEntity> {
        ...
        for (ExpandItem expandItem : expandOption.getItems()) {
            ...
            case STAEntityDefinition.THINGS:
                entity.setThing(getThingService()
                                                 .getEntityByIdRaw(entity.getThing()
                                                                         .getId(),
                                                                   expandItem.getQueryOptions()));
                break;
            default:
    ...
    private HistoricalLocationEntity updateEntity(String id, HistoricalLocationEntity entity)
    ...
    public HistoricalLocationEntity merge(HistoricalLocationEntity existing, HistoricalLocationEntity toMerge) {
        ...
        if (toMerge.getLocations() != null) {
            existing.getLocations()
                    .addAll(toMerge.getLocations());
        }
        return existing;
    }
    ...
    private void processLocations(HistoricalLocationEntity historicalLocation) throws STACRUDException {
        ...
        for (LocationEntity l : historicalLocation.getLocations()) {
            Optional<LocationEntity> location = locationRepository.findByStaIdentifier(l.getStaIdentifier(),
                                                                                       EntityGraphRepository.FetchGraph.FETCHGRAPH_HIST_LOCATIONS);
            if (location.isPresent()) {
                location.get()
                        .addHistoricalLocation(historicalLocation);
                locations.add(getLocationService().createOrUpdate(location.get()));
            } else {
                // We assume that l has historicalLocation linked to it correctly already and
                // just was not persisted
    ...
    private void updateThing(HistoricalLocationEntity historicalLocation) throws STACRUDException {
        getThingService().save(historicalLocation.getThing()
