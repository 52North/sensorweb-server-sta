...
public class ObservationQuerySpecifications extends EntityQuerySpecifications<DataEntity<?>> {
    ...
    public static Specification<DataEntity<?>> asSubject(final String relationIdentifier) {
        return (root, query, builder) -> {
            return builder.in(root.get(DataEntity.PROPERTY_SUBJECTS)).value(relationIdentifier);
        };
    }

    public static Specification<DataEntity<?>> withRelationStaIdentifier(final String relationIdentifier) {
        return (root, query, builder) -> {
