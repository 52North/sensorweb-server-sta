...
public class FormatService {
    ...
    public String createOrFetchFormat(String observationType) throws STACRUDException {
        Format formatPOJO = new Format();
        formatPOJO.setDefinition(observationType);
        synchronized (mutexFactory.getLock(formatPOJO.getDefinition())) {
            if (!formatDao.existsByFormat(formatPOJO.getDefinition())) {
                logger.debug("Persisting new formatPOJO: " + formatPOJO.getDefinition());
                return formatDao.save(formatPOJO);
            } else {
                return formatDao.findByFormat(formatPOJO.getDefinition()).get();
            }
        }
    }
}
