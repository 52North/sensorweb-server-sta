...
public abstract class CommonDatastreamService<S extends AbstractDatasetEntity, T extends StaIdentifierRepository<S> & AggregateRepository<S>>
        extends
        CommonSTAServiceImpl<T, DatastreamDTO, S> {
    ...
    protected S fetchExpandEntitiesWithFilter(S entity,
        ...
        for (ExpandItem expandItem : expandOption.getItems()) {
            ...
            switch (expandProperty) {
            case STAEntityDefinition.SENSOR:
                entity.setProcedure(getSensorService().getEntityByIdRaw(entity.getProcedure()
                                                                              .getId(),
                                                                        expandItem.getQueryOptions()));
                break;
            case STAEntityDefinition.THING:
                entity.setThing(getThingService().getEntityByIdRaw(entity.getThing()
                                                                         .getId(),
                                                                   expandItem.getQueryOptions()));
                break;
            case STAEntityDefinition.OBSERVED_PROPERTY:
                entity.setObservableProperty(getObservedPropertyService().getEntityByIdRaw(
                                                                                           entity.getObservableProperty()
                                                                                                 .getId(),
                                                                                           expandItem.getQueryOptions()));
                break;
            case STAEntityDefinition.OBSERVATIONS:
                // Optimize Request when only First/Last Observation is requested as we have
                // already fetched that.
                ...
                Page<DataEntity< ? >> observations = getObservationService()
                                                                            .getEntityCollectionByRelatedEntityRaw(entity.getStaIdentifier(),
                                                                                                                   ...
                                                                                                                   expandItem.getQueryOptions());
                entity.setObservations(observations.get()
    ...
    public S createOrfetch(S datastream) throws STACRUDException {
        if (!datastream.isProcessed()) {
            ...
            if (datastream.getStaIdentifier() != null && !datastream.isSetName()) {
                Optional<S> optionalEntity = getRepository()
                                                            .findOne(dQS.withStaIdentifier(datastream.getStaIdentifier()));
                if (optionalEntity.isPresent()) {
                    return optionalEntity.get();
                } else {
                    throw new STACRUDException(
                                               "No Datastream with id '"
                                                       + datastream.getStaIdentifier()
                                                       + "' "
                                                       + "found");
                }
            }
            ...
            synchronized (getLock(datastream.getStaIdentifier())) {
                ...
                AbstractDatasetEntity dataset = createandSaveDataset(datastream, null, datastream.getStaIdentifier());
    ...
    @Override
    public S updateEntity(String id, S entity, String method)
    ...
    private S updateEntity(String id, S entity) throws HibernateException, STACRUDException {
        synchronized (getLock(id)) {
            Optional<S> existing = getRepository().findOne(dQS.withStaIdentifier(id),
                                                           EntityGraphRepository.FetchGraph.FETCHGRAPH_UOM,
                                                           EntityGraphRepository.FetchGraph.FETCHGRAPH_OM_OBS_TYPE);
    ...
    @Override
    public S createOrUpdate(S entity) throws STACRUDException {
    ...
    /**
     * Constructs FilterPredicate based on given queryOptions. Additionally filters out Datasets that are
     * aggregated into DatasetAggregations.
     *
     * @param entityClass
    ...
    public S merge(S existing, S toMerge)
        ...
        if (toMerge.isSetOMObservationType()
                && !existing.getOMObservationType()
                            .getFormat()
                            .equals(toMerge.getOMObservationType()
                                           .getFormat())
                && !toMerge.getOMObservationType()
                           .getFormat()
    ...
    public void delete(String id) throws STACRUDException {
        synchronized (getLock(id)) {
            if (getRepository().existsByStaIdentifier(id)) {
                AbstractDatasetEntity datastream = getRepository().findByStaIdentifier(id)
                                                                  .get();
    ...
    private void check(AbstractDatasetEntity datastream) throws STACRUDException {
        if (datastream.getThing() == null
                || datastream.getObservableProperty() == null
    ...
    private void checkUpdate(AbstractDatasetEntity entity) throws STACRUDException {
        ...
        if (entity.getObservableProperty() != null
                && (entity.getObservableProperty()
                          .getIdentifier() == null
                        || entity.getObservableProperty()
                                 .isSetName()
    ...
    private void createOrfetchUnit(AbstractDatasetEntity datastream) throws STACRUDException {
        ...
        if (datastream.isSetUnit()) {
            synchronized (getLock(datastream.getUnit()
                                            .getSymbol()
                    + "unit")) {
                if (!unitRepository.existsBySymbol(datastream.getUnit()
                                                             .getSymbol())) {
                    unit = unitRepository.save(datastream.getUnit());
                } else {
                    unit = unitRepository.findBySymbol(datastream.getUnit()
                                                                 .getSymbol());
                }
    ...
    private boolean checkForFirstLastObservation(ExpandItem expandItem) {
        return expandItem.getQueryOptions()
                         .hasTopFilter()
                && expandItem.getQueryOptions()
                             .getTopFilter()
                             .getValue() == 1
    ...
    private boolean checkPhenomenonTime(ExpandItem expandItem) {
        return expandItem.getQueryOptions()
                         ...
                         .getSortProperties()
                         .stream()
                         .filter(p -> p.getValueReference()
                                       .equals(StaConstants.PROP_PHENOMENON_TIME))
                         .findAny()
                         .isPresent();
    }

    private SortOrder getSortOrder(ExpandItem expandItem) {
        for (OrderProperty orderProperty : expandItem.getQueryOptions()
                                                     .getOrderByFilter()
                                                     .getSortProperties()) {
            if (orderProperty.getValueReference()
                             .equals(StaConstants.PROP_PHENOMENON_TIME)) {
