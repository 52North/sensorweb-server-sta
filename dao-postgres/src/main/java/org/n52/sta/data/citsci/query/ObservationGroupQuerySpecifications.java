...
public class ObservationGroupQuerySpecifications extends EntityQuerySpecifications<ObservationGroupEntity> {
    ...
    public static Specification<ObservationGroupEntity> withRelationStaIdentifier(final String relationIdentifier) {
        return (root, query, builder) -> {
            final Join<ObservationGroupEntity, ObservationRelationEntity> join =
                root.join(ObservationGroupEntity.PROP_OBSERVATIONRELATIONS, JoinType.INNER);
            return builder.equal(join.get(DescribableEntity.PROPERTY_STA_IDENTIFIER), relationIdentifier);
        };
    }

    public static Specification<ObservationGroupEntity> withLicenseStaIdentifier(final String relationIdentifier) {
        return (root, query, builder) -> {
            final Join<ObservationGroupEntity, LicenseEntity> join =
                root.join(ObservationGroupEntity.PROPERTY_LICENSE, JoinType.INNER);
            return builder.equal(join.get(DescribableEntity.PROPERTY_STA_IDENTIFIER), relationIdentifier);
        };
    }

    public static Specification<ObservationGroupEntity> withObservationStaIdentifier(final String observationId) {
        return (root, query, builder) -> {
            final Join<ObservationGroupEntity, DataEntity> join =
