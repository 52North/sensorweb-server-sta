...
public class PartyQuerySpecification extends QuerySpecification<PartyEntity> {

    public PartyQuerySpecification() {
        super();
        ...
        this.filterByMember.put(StaConstants.THINGS, createThingsFilter());
        ...
        this.entityPathByProperty.put(StaConstants.PROP_AUTH_ID,
                new SimplePropertyComparator<>(PartyEntity.PROPERTY_IDENTIFIER));
        this.entityPathByProperty.put(StaConstants.PROP_ROLE,
                new SimplePropertyComparator<>(PartyEntity.PROPERTY_ROLE));
    }
    ...
    private MemberFilter<PartyEntity> createDatastreamsFilter() {
    ...
    private MemberFilter<PartyEntity> createGroupsFilter() {
        return specification -> (root, query, builder) -> {
            EntityQuery memberQuery = createQuery(IdEntity.PROPERTY_ID, GroupEntity.class);
            Subquery< ? > subquery = memberQuery.create(specification, query, builder);
            // m..n
            Join< ? , ? > join = root.join(PartyEntity.PROPERTY_GROUPS, JoinType.INNER);
            return builder.in(join.get(IdEntity.PROPERTY_ID))
                          .value(subquery);
        };
    }

    private MemberFilter<PartyEntity> createThingsFilter() {
        return specification -> (root, query, builder) -> {
            EntityQuery memberQuery = createQuery(IdEntity.PROPERTY_ID, PlatformEntity.class);
            Subquery< ? > subquery = memberQuery.create(specification, query, builder);
            // m..n
            Join< ? , ? > join = root.join(PartyEntity.PROPERTY_PLATFORMS, JoinType.INNER);
            return builder.in(join.get(IdEntity.PROPERTY_ID))
                          .value(subquery);
        };
    }
