...
public class ObservationQuerySpecifications extends EntityQuerySpecifications<DataEntity<?>> {
    ...
    public static Specification<DataEntity<?>> withDatastreamStaIdentifier(
        ...
        return (root, query, builder) -> {
            ...
            subquery.select(realDataset.get(AbstractDatasetEntity.PROPERTY_ID))
                    .where(builder
                            .equal(realDataset.get(AbstractDatasetEntity.PROPERTY_STA_IDENTIFIER),
    ...
    @Override
    protected Specification<DataEntity<?>> handleRelatedPropertyFilter(
            String propertyName,
        ...
        return (root, query, builder) -> {
            try {
                if (StaConstants.DATASTREAM.equals(propertyName)) {
                    ...
                    sq.select(datastream.get(AbstractDatasetEntity.PROPERTY_ID))
                            .where(
                    ...
                    subquery.select(feature)
                            .where(
                                    ((Specification<FeatureEntity>) propertyValue).toPredicate(feature, query,
                                            builder));
                    sq.select(dataset.get(DatasetEntity.PROPERTY_ID))
                            .where(builder.in(dataset.get(DatasetEntity.PROPERTY_FEATURE))
                                    .value(subquery));
                    return builder.in(root.get(DataEntity.PROPERTY_DATASET))
                            .value(sq);
                ...
                switch (propertyName) {
                    ...
                    case StaConstants.PROP_RESULT:
                        if (propertyValue.getJavaType()
                            ...
                            return builder.and(
                                    builder.or(countPred,
                                            root.get(DataEntity.PROPERTY_VALUE_COUNT)
                                                    .isNull()),
                                    builder.or(quantityPred,
                                            root.get(DataEntity.PROPERTY_VALUE_QUANTITY)
                                                    .isNull()),
                                    root.get(DataEntity.PROPERTY_VALUE_CATEGORY)
                                            .isNull(),
                                    root.get(DataEntity.PROPERTY_VALUE_TEXT)
                                            .isNull(),
                                    root.get(DataEntity.PROPERTY_VALUE_BOOLEAN)
                                            .isNull());
                        } else if (propertyValue.getJavaType()
                             ...
                             * root.get(ObservationEntity.PROPERTY_VALUE_BOOLEAN), propertyValue, operator,
                             * builder, false);
                             */

                            // Check for category, text, boolean as those represented by String in query
                            // Do not return observations with numeric result type as we are filtering on
                            // string
                            return builder.and(
