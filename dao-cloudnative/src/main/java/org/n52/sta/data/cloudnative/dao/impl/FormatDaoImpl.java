...
public class FormatDaoImpl {
    ...
    public Optional<String> findByFormat(String definition) {
        Condition predicate = StaEntity.FORMAT.DEFINITION.eq(definition);
        Result<Record> result = ctx.select().from(StaEntity.UNIT).where(predicate).fetch();
        return Optional.ofNullable(mapResultToFormat(result)).map(dto -> dto.get(0).getDefinition());
    }

    private List<Format> mapResultToFormat(Result<Record> result) {
        return null;
    }

    public String save(Format formatPOJO) {
        // TODO:
        formatPOJO.setFormatId(0L);
        return formatPOJO.getDefinition();
    }
}
